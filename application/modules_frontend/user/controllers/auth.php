...
//		$users = $this->users->check_username('ducbui');
		
//		$user = new users();
//		$temp = $user->check_username()
		$temp = $this->user_temp->create_temp(array());
...
//				redirect('', 'location');
				redirect('user/auth/custom_permissions', 'location');
			}
			else
			{
				...
				{						
					...
					if ($this->dx_auth->is_max_login_attempts_exceeded())
					{
						...
						$this->_data['show_captcha'] = TRUE;
					}
					
					// Load login page view
					$this->loadView($this->dx_auth->login_view);
				}
			}
		}
		else
		{
			$this->_data['auth_message'] = 'You are already logged in.';
			$this->loadView($this->dx_auth->logged_in_view);
	...
	{
		$this->dx_auth->logout();
		$this->_data['auth_message'] = 'You have been logged out.';		
		$this->loadView($this->dx_auth->logout_view);
	}
	...
	{		
		...
		{	
			...
			{	
				...
				{
					$this->_data['auth_message'] = 'You have successfully registered. Check your email address to activate your account.';
				}
				else
				{					
					$this->_data['auth_message'] = 'You have successfully registered. '.anchor(site_url($this->dx_auth->login_uri), 'Login');
				}
				
				// Load registration success page
				$this->loadView($this->dx_auth->register_success_view);
			}
			else
			{
				...
				$this->loadView($this->dx_auth->register_view);
			}
		}
		elseif ( ! $this->dx_auth->allow_registration)
		{
			$this->_data['auth_message'] = 'Registration has been disabled.';
			$this->loadView($this->dx_auth->register_disabled_view);
		}
		else
		{
			$this->_data['auth_message'] = 'You have to logout first, before registering.';
			$this->loadView($this->dx_auth->logged_in_view);
		}
	}
	...
	function register_recaptcha()
	{
		...
		{	
			...
			{
				// Load registration page
				$this->loadView('user/auth/register_recaptcha_form');
			}
		}
		elseif ( ! $this->dx_auth->allow_registration)
		{
...
//		$key = $this->uri->segment(4);
		...
		{
			$this->_data['auth_message'] = 'Your account have been successfully activated. '.anchor(site_url($this->dx_auth->login_uri), 'Login');
			$this->loadView($this->dx_auth->activate_success_view);
		}
		else
		{
			$this->_data['auth_message'] = 'The activation code you entered was incorrect. Please check your email again.';
			$this->loadView($this->dx_auth->activate_failed_view);
		}
	}
	...
	function forgot_password()
	{
		...
		{
			$this->_data['auth_message'] = 'An email has been sent to your email with instructions with how to activate your new password.';
			$this->loadView($this->dx_auth->forgot_password_success_view);
		}
		else
		{
			$this->loadView($this->dx_auth->forgot_password_view);
		}
	}
	...
	function reset_password()
	{
		...
		{
			$this->_data['auth_message'] = 'You have successfully reset you password, '.anchor(site_url($this->dx_auth->login_uri), 'Login');
			$this->loadView($this->dx_auth->reset_password_success_view);
		}
		else
		{
			$this->_data['auth_message'] = 'Reset failed. Your username and key are incorrect. Please check your email again and follow the instructions.';
			$this->loadView($this->dx_auth->reset_password_failed_view);
		}
	}
	...
	function change_password()
	{
		...
		{			
			...
			{
				$this->_data['auth_message'] = 'Your password has successfully been changed.';
				$this->loadView($this->dx_auth->change_password_success_view);
			}
			else
			{
				$this->loadView($this->dx_auth->change_password_view);
			}
		}
		else
		{
	...
	function cancel_account()
	{
		...
		{			
			...
			{
				$this->loadView($this->dx_auth->cancel_account_view);
